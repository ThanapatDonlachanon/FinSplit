...
export async function GET(req: NextRequest) {
  ...
  const [wallets, categories, transactions, budgets, sessions] = await Promise.all([
  ...
  return NextResponse.json({
    ...
    sessions: sessions.map((s) => {
      const meMember = s.members.find((m) => m.isMe);
      return {
        id: s.id,
        name: s.name,
        emoji: s.emoji,
        ...
        bills: s.bills.map((b) => ({
          ...
          mode: b.mode,
          paidBy: (() => {
            try { return JSON.parse(b.paidBy); } catch { return b.paidBy; }
          })(),
